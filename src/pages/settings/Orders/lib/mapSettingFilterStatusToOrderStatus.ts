...
export const mapSettingFilterStatusToOrderStatus = (status: SettingsOrderStatusFilterType) => {
    ...
    if (status === SettingsOrdersStatusFilter.CANCELLED) return [OrderStatus.CANCELLED, OrderStatus.REFUNDED]
    if (status === SettingsOrdersStatusFilter.COMPLETED) return [OrderStatus.DELIVERED]
    if (status === SettingsOrdersStatusFilter.IN_PROGRESS) return [OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PROCESSING, OrderStatus.SHIPPED]
}
