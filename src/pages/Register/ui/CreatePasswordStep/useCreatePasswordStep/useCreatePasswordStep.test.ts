...
import {FormSteps} from "../../../config/formSteps.ts";

import {useCreatePasswordStep} from "./useCreatePasswordStep.ts";
...
vi.mock("../../../model/registerFlowContext.ts", () => ({
    useRegisterFlow: () => ({
        form: {
...
describe("useCreatePasswordStep", () => {
    ...
    test("submitPassword sets field error for invalid password", async () => {
        const {result} = renderHook(() => useCreatePasswordStep());
