...
import {
    ...
    POLLING_INTERVAL_MS,
    TERMINAL_STATUSES,
} from "@/pages/CheckoutResult/config/consts.ts";
import {resolvePaymentStatus} from "@/pages/CheckoutResult/lib/resolvePaymentStatus.ts";
import {CheckoutSessionStatus} from "@/pages/CheckoutResult/model/types/checkoutResultTypes.ts";

import {clearCartState} from "@/entities/cart";

import {useAppDispatch} from "@/shared/lib/state";

interface UseCheckoutResultPageParams {
    sessionId: string | null;
}

export const useCheckoutResultPage = ({sessionId}: UseCheckoutResultPageParams) => {
