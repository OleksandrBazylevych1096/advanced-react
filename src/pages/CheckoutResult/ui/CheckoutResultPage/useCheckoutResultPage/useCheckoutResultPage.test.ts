...
import {useCheckoutResultPage} from "./useCheckoutResultPage.ts";
...
describe("useCheckoutResultPage", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    test("marks session as resolved on paid status", () => {
        ...
        const {result} = renderHook(() => useCheckoutResultPage({sessionId: "session-1"}));
