...
import {useChooseDeliveryDate} from "./useChooseDeliveryDate.ts";
...
vi.mock("../../../api/chooseDeliveryDateApi.ts", () => ({
...
describe("useChooseDeliveryDate", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    test("sets first date when slots loaded", async () => {
        const {result} = renderHook(() => useChooseDeliveryDate());
    ...
    test("keeps selected date during slots loading with temporary empty data", async () => {
        const {result, rerender} = renderHook(() => useChooseDeliveryDate());
    ...
    test("requests saved selection for authenticated user even when modal is closed", () => {
        renderHook(() => useChooseDeliveryDate());
